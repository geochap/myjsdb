...
var Store = exports.Store = function(name, options){
    ...
    this.FLAG_NON_NULL_PROP_VAL = 0;
    this.FLAG_NULL_PROP_VAL = 1;
    this.FLAG_OBJECT = 2;
    this.FLAG_ARRAY = 4;
    this.FLAG_ROOT = 8;
...
//        console.log(query.getSql()); 
        this.client.query(query.getSql(), function(err, res){
            fn(err, err?null:new Sql2JsonHelper(self, res).getResults(desc));
        });
    }
    ...
    this.getDocument = function(name, fn){
        ...
        this.client.query('select a.* from ' + this.name + '_json a join ' + this.name + '_json_doc b on a.docid = b.id'
            + ' where b.name=?', [name], function(err, res){
                fn(err, err?null:new Sql2JsonHelper(self, res).getRoot());
            });
    }
...
var Sql2JsonHelper = function(store, res){
    this.objects = {};
    this.solutions = {};
    var self = this;
    var root = null;
...
var Solution = function(){
    ...
    this.getValue = function(obj){
        ...
        if (obj instanceof PropVal){
            ...
            if (objref != null && objref[obj.name])
                return objref[obj.name];
            return obj.value;
        }
...
;
    ...
    var handleConstraint= function(constraint){
        ...
        if (typeof pv.value != 'function'){
            if (pv.value == null)
                conditions.push(table + '.flag=' + options.store.FLAG_NULL_PROP_VAL);
            else {
    ...
    var getColumn = function(pv, constraint){
        if (typeof constraint.arg2 == 'string' || pv.value == String)
            return ".string_value";
        if (typeof constraint.arg2 == 'number' || pv.value == Number)
            return ".number_value";
        if (typeof constraint.arg2 == "boolean" || pv.value == Boolean)
            return ".bool_value";
